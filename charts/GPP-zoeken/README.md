...
| settings.job.waitForElasticTimeout | int | `300` | Seconds to wait for the Elasticsearch cluster to accept connections before initializing the indices. On a fresh install this Job is created in the same release as the cluster, so it starts before Elasticsearch is listening. Zero waits indefinitely. |
